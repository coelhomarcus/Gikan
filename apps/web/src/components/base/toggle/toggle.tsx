...
export const ToggleBase = ({ className, isDisabled, isFocusVisible, isSelected, slim, size = "sm" }: ToggleBaseProps) => {
    ...
    return <span aria-hidden className={cx("flex cursor-pointer rounded-full bg-layer-2 ring-[0.5px] ring-subtle outline-accent-strong transition duration-150 ring-inset", isSelected && "bg-accent-primary", isDisabled && "cursor-not-allowed opacity-50", isFocusVisible && "outline-2 outline-offset-2", slim && "ring-1", root, className)}><span className={cx("rounded-full bg-fg-white shadow-sm transition-transform", slim && "border border-toggle-border shadow-xs", thumb, isSelected && (size === "sm" ? "translate-x-4" : "translate-x-5"))} /></span>;
};
