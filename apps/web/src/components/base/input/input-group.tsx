...
export const InputGroup = ({ size = "md", prefix, leadingAddon, trailingAddon, label, hint, hideRequiredIndicator, children, ...props }: InputGroupProps) => {
    ...
    const paddings = sortCx({
        sm: {
            input: cx(
                hasLeading && "pr-9 pl-2",
                hasTrailing && (prefix ? "pr-6 pl-0" : "pr-6 pl-3"),
            ),
            leadingText: "pr-1.5 pl-3",
        },
        md: {
            input: cx(
                hasLeading && "pr-9 pl-2.5",
                hasTrailing && (prefix ? "pr-6 pl-0" : "pr-6 pl-3"),
            ),
    ...
    return (
        <TextField
            ...
            tooltipClassName={cx(hasTrailing && !hasLeading && "right-0")}
            wrapperClassName={cx(
                "z-10",
