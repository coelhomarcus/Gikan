...
export const ButtonUtility = ({ tooltip, className, isDisabled, icon: Icon, size = "sm", color = "secondary", tooltipPlacement = "top", ...otherProps }: Props) => {
    ...
    const content = href ? (
        <a {...(otherProps as LinkProps)} href={isDisabled ? undefined : href} aria-disabled={isDisabled || undefined} className={cx("group relative inline-flex h-max cursor-pointer items-center justify-center rounded-md p-1.5 outline-focus-ring transition duration-100 ease-linear", isDisabled && "pointer-events-none opacity-50", styles[color], "*:data-icon:pointer-events-none *:data-icon:shrink-0 *:data-icon:text-current", size === "xs" ? "*:data-icon:size-3.5" : "*:data-icon:size-4", className)}>
            {isReactComponent(Icon) && <Icon data-icon />}{isValidElement(Icon) && Icon}
        </a>
    ) : (
        <BaseButton {...(otherProps as ButtonProps)} disabled={isDisabled} aria-label={tooltip} className={cx("group relative inline-flex h-max cursor-pointer items-center justify-center rounded-md p-1.5 outline-focus-ring transition duration-100 ease-linear disabled:cursor-not-allowed disabled:opacity-50", styles[color], "*:data-icon:pointer-events-none *:data-icon:shrink-0 *:data-icon:text-current", size === "xs" ? "*:data-icon:size-3.5" : "*:data-icon:size-4", className)}>
            {isReactComponent(Icon) && <Icon data-icon />}{isValidElement(Icon) && Icon}
        </BaseButton>
    );
