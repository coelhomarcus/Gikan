...
export const MobileNavigationHeader = ({ children }: PropsWithChildren) => (
    <Dialog.Root>
        <header className="flex h-14 items-center justify-between border-b border-subtle bg-surface-1 p-3 pl-4 lg:hidden">
            <GikanLogo className="h-6" />
            <Dialog.Trigger render={<button type="button" aria-label="Expand navigation menu" className="group flex size-9 items-center justify-center rounded-md bg-surface-1 text-secondary outline-accent-strong hover:bg-layer-1-hover focus-visible:outline-2 focus-visible:outline-offset-2" />}>
        ...
        <Dialog.Portal>
            ...
            <Dialog.Viewport className="fixed inset-0 z-50 flex lg:hidden">
                <Dialog.Popup className="relative w-full max-w-74 cursor-auto bg-surface-1 shadow-xl">
